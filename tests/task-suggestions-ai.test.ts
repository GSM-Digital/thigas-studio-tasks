  ...
  it("normaliza bônus, penalidades e linguagem do contrato", async () => {
    const client: StructuredGenerationClient = {
      generateStructured: vi.fn().mockResolvedValue({ sugestoes: [
        { titulo: "Fazer cópia de segurança", descricao: "Salve todos os arquivos antes de começar a migração.", categoria: "essential", percentual_bonus: 5, percentual_penalidade_omissao: 2, exige_evidencia: false, ferramentas: ["Supabase", "Supabase"] },
        { titulo: "Validar os formulários", descricao: "Envie um teste e confirme se o contato chegou corretamente.", categoria: "recommended", percentual_bonus: 4, percentual_penalidade_omissao: 12, exige_evidencia: true, ferramentas: [] },
    ...
    expect(suggestions).toEqual([
      expect.objectContaining({ category: "essential", rewardPercentage: 3, omissionPenaltyPercentage: 5, evidenceRequired: true, tools: ["Supabase"] }),
      expect.objectContaining({ category: "recommended", rewardPercentage: 3, omissionPenaltyPercentage: 0, evidenceRequired: true }),
